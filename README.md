...
### Dashboard
The primary view showing real-time network throughput, an ambient waveform of current activity, physical interface counters, and cumulative session totals.

### Activity
Displays a live list of applications consuming bandwidth, broken down by download and upload transfer rates.
...
### Mini
A small floating instrument that stays out of your way. Drag it anywhere across multiple monitors to keep a continuous eye on live speeds. Clicking it restores the full dashboard.
...
- **Diagnostics**: Unhandled crash errors may be recorded locally in `%LOCALAPPDATA%\TRACE\app_activity.log` to aid troubleshooting.
...
- **Administrator**: Attributing bandwidth to individual processes relies on Windows Event Tracing for Windows (ETW kernel network events), which requires Administrator elevation. When run without elevation, system throughput remains fully functional while process attribution displays an informational notice.
...
- **Runtime**: Self-contained (no separate .NET runtime installation required for standalone builds)
